...
agent: "agent"
tools: ["search", "codebase", "editFiles", "createFile"]
---
...
3. **Analizar el código fuente** en `app/` para detectar:
   - Ramas de código no cubiertas por tests existentes (`if/else`, `except`, early returns)
   - Funciones, handlers o utilidades sin tests asociados
   - Nuevos endpoints, servicios o módulos que no tengan archivo de test correspondiente

4. **Generar tests** que cubran cada sección del PRD y cualquier código no cubierto:
...
2. **Actualiza los archivos de test existentes** (`tests/test_urls.py`, `tests/test_redirect.py`, `tests/test_analytics.py`, `tests/test_qrcode.py`, `tests/test_rate_limiting.py`, `tests/test_services.py`, `tests/test_expiration.py`, `tests/test_health.py`) con los tests generados. Si un archivo no existe, créalo.
3. **Crea nuevos archivos de test** (`tests/test_<dominio>.py`) cuando:
   - Se detecte un módulo en `app/` sin tests correspondientes (ej: nuevo router, servicio o utilidad).
   - El PRD documente funcionalidad que no encaje en ningún archivo de test existente.
   - El análisis de cobertura revele módulos o ramas sin cubrir que requieran su propio archivo de test.
   - Los nuevos archivos deben seguir la misma estructura: imports propios, clases agrupadas por feature con `@pytest.mark.asyncio`, y reutilizar `conftest.py`.
4. **No modifiques `tests/conftest.py`** a menos que sea estrictamente necesario agregar una nueva fixture requerida por los tests nuevos.
5. **Confirma** al usuario qué archivos fueron creados, cuáles actualizados, y cuántos tests nuevos se añadieron en cada uno.
